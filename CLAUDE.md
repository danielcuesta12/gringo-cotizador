...
~40 archivos en `install/`. Al desplegar hay que aplicar las **nuevas** en phpMyAdmin (no hay tracking automático). **Para saber cuáles faltan: pega `install/check_migraciones.sql` en phpMyAdmin** → te lista cada migración con ✅/❌ (mirando si su columna/tabla existe). Las más recientes: `nubefact.sql`, `nubefact_email.sql`, `multilocal_facturacion.sql`, `gastos.sql`, `49_almacen_central.sql` (ubicaciones.es_almacen), `50_quotes_evento.sql` (quotes.evento_nombre/evento_atendido), `51_agenda_atendido.sql`, `52_quotes_venta.sql` (quotes.venta_real, **sin uso** — quedó en agenda), `53_agenda_venta.sql` (agenda.venta_real), `54_eventos_usa_pos.sql` (eventos.usa_pos DEFAULT 1). Muchas tablas tienen guards (`inventarioListo()`, `$ready`, try/catch) para no romper si falta aplicar una.
