...
- **Estado (jun 2026) — DESPLEGADO / al día:** en `main` y **aplicado en el server** (migraciones `60_costeo_recetas.sql` y `61_subrecetas_stock.sql` ya corridas en phpMyAdmin): **Costeo Fase 1+2** (recetas con subrecetas + ficha + simulador food cost + dashboard de costeo), **subrecetas con stock/producción** y **boleta simple visible en el POS**. Código, docs y BD del server **al día**. **Pendiente de uso real:** probar el circuito completo con datos (crear receta → subreceta con "lleva stock" → producir un lote en `admin/inventory/produccion.php` → vender → marcar **"Listo"** en el KDS, que descuenta insumos + stock de subreceta → ver el **dashboard de Costeo** y el **stock** con alerta de negativo). Lo siguiente grande en cola es **PedidosYa** (abajo), bloqueado por credenciales.
