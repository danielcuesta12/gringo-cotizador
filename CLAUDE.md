...
**Inventario:** `insumos`, `insumo_stock`, `recetas` (legado, ver costeo), `inventario_movimientos`, `proveedores`, `compras`, `compra_items`. **Costeo (Fase 1):** `subrecetas` + `subreceta_items` (preparaciones base = atado de insumos con rendimiento), `receta_componentes` (receta de un producto = insumos **y/o** subrecetas; reemplaza a `recetas`), `receta_ficha` (ficha técnica por producto), `subreceta_stock` + `subreceta_movimientos` (stock propio de subrecetas con `lleva_stock`, migración 61).
...
**Claves (32):** `dashboard, quotes, events, calendar, clients, requests, reservas, analytics` · `products, categories, packages, modifiers, locations` · `pedidos, kds, pos_terminal, pos_metodos, pos_caja, pos_monitor, pos_clientes, mesas` · `inv_insumos, inv_stock, inv_recetas, inv_movimientos, inv_compras, inv_evento, inv_costeo` · `cartas_pdf, qr, landing` · `gastos, asistencia` (Finanzas/Asistencia). (`settings`/`users`/`facturacion` = solo `isAdmin()`.) La clave `mesas` gatea el **admin** del plano (`admin/mesas/`); la **app del mozo** NO usa este sistema — entra por **PIN de empleado** (`empleados.pin_hash`), no por permiso de `users`.
...
- **Cuenta = comandas:** abrir cuenta en una mesa → cada "Enviar a cocina" crea una **comanda** = un `pedido` (`origen='mesa'`, `cuenta_id`, `mesa_id`, `estado='en_preparacion'`) que entra al KDS con badge **MESA N · Ronda**. Catálogo con búsqueda + **agregar de un toque con contador inline** (− n + por producto; los que tienen modificadores muestran badge + abren el modal); borrador editable; anular ítem/comanda con motivo (solo antes de "Listo"). Reintento ante caída de red al enviar.
- **Aviso de "Listo":** cuando cocina marca un pedido `listo`, el mozo **dueño de esa mesa** recibe **banner (arriba) + ding**; es notificación, no estado a cerrar. (`plano_estados` devuelve `listos` filtrado por `empleado_id`; anti-spam: siembra los listos al abrir y solo avisa transiciones nuevas.) Toggle de sonido **por color** (verde "Sonido" / rojo "Silencio"). El ding es WebAudio (sin descargas): hay que **desbloquear el AudioContext en el primer gesto** y **iOS lo calla con el switch físico de silencio** (limitación del SO, no bug).
- **Precuenta** no fiscal imprimible (RawBT, `pos/escpos_build.php` → `escposPrecuentaBytes`); marca la mesa en rosa.
- **Cobro (Sub-build C):** precuenta → **4 modos de split** (todo/iguales/por ítems/montos libres) → cada parte con **pago mixto** (varias líneas método+monto) + **comprobante** opcional (ticket/boleta/factura por NubeFact). El dinero va a **`cuenta_pagos`** (única fuente de verdad; nunca `cuentas.total`). El mozo cobra → entra al **turno de caja abierto del local** (sin caja abierta = bloquea); el arqueo (`cerrar_turno`) y el dashboard suman `cuenta_pagos` (las comandas `origen='mesa'` se excluyen del SUM de `pedidos`, sin doble conteo). Estados de mesa: ocupada · precuenta (rosa) · por_cobrar (parcial). **UX del cobro:** descuento detrás de un link → **modal** (no inline; `faltaActual()` ya incluye el descuento, así partes y cuadre se ajustan solos); modo de cobro en **grid 2×2**; un único stepper **`cobro.nPartes`** ("¿en cuántas partes?") para iguales/ítems/montos; dentro de cada parte la **última línea de pago es balanceadora** (solo-lectura = total de la parte − suma de las demás; se recalcula al editar/agregar).
...
**Costeo + subrecetas (Fase 1, migración 60).** Las recetas dejaron de ser `recetas (product_id,insumo_id,cantidad)` y pasaron a `receta_componentes (product_id, tipo insumo|subreceta, ref_id, cantidad)` (con backfill de lo existente como `insumo`). Una **subreceta** (`subrecetas`+`subreceta_items`) es una preparación base reusable (salsa, masa, aderezo) hecha de insumos, con `rendimiento`: su **costo/UM** = costo total de insumos / rendimiento. Las subrecetas que **no llevan stock** (toggle `lleva_stock=0`, por defecto) **explotan a insumos** al costear y al vender, vía `recetaExplotaInsumos($productId)` (`insumo_id => cantidad`, suma `(item.cantidad/rendimiento)×componente.cantidad`). Lo usan `recetaCosto` (costeo), `eventoConsumoTeorico` y `salida_evento.php` (foodtruck). La **venta** (`descontarStockPedido`, venta/KDS Listo) usa `recetaConsumo()`, que **reparte**: insumos directos y subrecetas-sin-stock van a insumos; subrecetas-con-stock se descuentan de su propio stock (ver «Subrecetas con stock» abajo). Helpers en `includes/inventario.php` (`subrecetasListo`, `recetaComponentesListo`, `subrecetaCostoTotal/UM`, `recetaComponentes`); matemática pura de food cost en `includes/costeo.php` (`foodCostCalc`, `precioSugerido`, `fcClase` — con tests en `tests/costeo_test.php`). **Páginas:** `admin/inventory/subrecetas.php` (lista) + `subreceta_form.php` (editor con búsqueda en vivo + crear insumo al vuelo + costo/UM en vivo); `receta_form.php` ampliado: componentes mixtos (insumo|subreceta vía `api/insumos.php?action=componentes_buscar`), **ficha técnica** imprimible y **simulador de food cost** (precio jugable + semáforo ≤35/≤42 + precio sugerido por food cost objetivo — **solo simulación, no persiste**; IGV de `getSetting('igv_pct')`). Todo bajo permiso `inv_recetas`. **Dashboard de Costeo (Fase 2):** `admin/inventory/costeo.php` (permiso `inv_costeo`) — solo lectura: KPIs (food cost promedio, nº platos con receta, alertas fc>35%) + **ranking de platos por food cost** ascendente (menor = más rentable) con semáforo, precio del local principal (`es_principal`; "sin precio" se excluye del promedio/alertas), filtros por categoría/búsqueda.

**Subrecetas con stock (producción por lotes, migración 61).** Opt-in por subreceta (`subrecetas.lleva_stock`): las marcadas se **producen por lote** y llevan **stock propio** por local (`subreceta_stock`, puede ir **negativo**) con ledger espejo (`subreceta_movimientos`). Las **no** marcadas siguen explotando a insumos (comportamiento por defecto). **El costeo NO cambia: sigue siendo teórico** (`subrecetaCostoUM` desde el costo vivo de insumos); el stock es solo para inventario/operación. Helpers en `includes/inventario.php`: `subrecetaStockListo`, `subMovimiento` (espejo de `invMovimiento`), `subProducir` (transaccional: consume insumos `item×lotes` y suma `rendimiento×lotes`), `subTransferir` (despacho entre locales), `recetaConsumo` (reparto venta, sobre la función pura `repartirConsumo` de `includes/costeo.php` + tests `tests/subreceta_consumo_test.php`). **Producción multi-local** en `admin/inventory/produccion.php` (cualquier `ubicacionesConInventario()`, no solo el almacén central). **Despacho** de subrecetas integrado en `admin/inventory/operar.php` (modo Salidas del almacén). Stock visible en `admin/inventory/stock.php` (sección Subrecetas con alerta de negativo/bajo mínimo). Toggle "se produce y lleva stock" en el editor de subreceta. Páginas de producción/despacho/stock bajo `inv_stock`. **Nunca bloquea la venta:** si falta stock de subreceta, vende y deja negativo (la alerta). **Fuera de alcance:** stock de subrecetas en eventos/foodtruck (`salida_evento`/`eventoConsumoTeorico` siguen explotando a insumos), merma de producción, editar stock_min de subrecetas.
...
**Boleta simple (sin documento):** `nubefactEmitir` emite boleta **sin DNI** como **"CLIENTE VARIOS"** (`cliente_tipo_de_documento='-'`, `numero='0'`) automáticamente: en el POS elegís "Boleta" y dejás el documento **vacío** → sale boleta simple (no hay validación que lo bloquee; el modal lo rotula con placeholder dinámico + nota "vacío = boleta simple"). Factura sí exige **RUC de 11 dígitos**. **Umbral SUNAT:** la boleta sin identificar al cliente solo corresponde **bajo cierto monto por operación** (viene siendo **S/ 700**); por encima SUNAT exige DNI (confirmar el tope vigente con el contador).
...
| Aviso del mozo no sonaba (móvil) | desbloquear el `AudioContext` en el primer gesto (touch/click) + reproducir un buffer silencioso (iOS); el ding nace `suspended` si se crea fuera de un gesto. iOS igual lo calla con el switch de silencio |
| Mozo: el toast tapaba el footer "Ver borrador" | mover `.toast` de `bottom` a `top` (debajo del header), no choca con footer ni sheets |
...
## Proyecto SaaS — "YupiPOS" (yupipos.com) — EN CONSTRUCCIÓN

> **Encuadre:** El Gringo **sigue en cPanel intacto, NO se toca.** El SaaS es un **producto NUEVO y separado**: tomar este **esqueleto como punto de partida**, modernizarlo y hacerlo **a prueba de todo para venderlo** a otros restaurantes. **Marca DECIDIDA: YupiPOS · dominio `yupipos.com`** (jun 2026); el nombre técnico interno también es `yupipos` (carpetas `yupipos-web` / `yupipos-app`, BD `yupipos_central`). Brief original e infografías en `docs/saas-mipospro/`.
...
**White-label — REQUISITO de arquitectura.** La **app del producto nace en colores GENÉRICOS/neutros**; cada cliente **monta su color, logo y nombre** (theming por tenant, leído de su propia BD). **NUNCA hardcodear colores** — todo por variables (la semilla ya existe aquí: `brandHead()` + `company_settings.brand_*`, ver "Marca EDITABLE"). OJO: esto aplica a la **app que monta cada restaurante**, NO a la **web comercial de YupiPOS** (esa tiene su propia marca fija = ámbar `#ea580c`).
...
**Módulos portados + auditados (jun 2026).** Ya se portaron los 23 módulos (Cotizaciones B2B, Solicitudes, Reservas, Calendario/Eventos, Carta pública, POS, KDS, Pedidos, Mesas/Mozo, Catálogo, Inventario, Recetas/Costeo, Compras, Salida a evento, Landing, Analítica, Cartas PDF, Gastos, Facturación, Asistencia, Izipay, Ajustes/Usuarios/Permisos, Dashboard) como Filament Resources/Pages + Services + controllers públicos. Se corrió una **auditoría de paridad 1:1** (workflow multi-agente, 46 agentes, cada brecha verificada adversarialmente) → **`yupipos-app/docs/auditoria-paridad-elgringo.md`**.

**Auditoría → 170 brechas (2 críticas · 33 altas · 69 medias · 66 bajas), TODAS cerradas** (3 olas de workflows por severidad, cada agente replicó El Gringo 1:1 y commiteó por módulo). Detalle en `yupipos-app/docs/auditoria-paridad-elgringo.md`. Regla [[yupipos-paridad-exacta]].

**EN PRODUCCIÓN (jun 2026).** YupiPOS está **desplegado y en vivo**. VPS Hostinger (KVM 2, Ubuntu 24.04, IP 2.25.67.143, `ssh yupipos`). Stack: Nginx + PHP 8.5 (memory_limit 512M) + MariaDB + Redis + Supervisor (2 queue workers) + backups diarios; hardening ufw+fail2ban+SSH-key. **Skill que automatiza todo el ciclo: `~/.claude/skills/yupipos-deploy/`** (provisión/deploy/SSL/workers/backup idempotentes; re-deploy = `git push` + `03-deploy.sh`). Dominio en GoDaddy con DNS en **Cloudflare**: `yupipos.com`=landing (Vercel) · `app.yupipos.com`=panel super-admin · `*.yupipos.com`=tenants, todo con HTTPS (cert de origen autofirmado TEMPORAL; falta el Origin Cert de Cloudflare para Full-strict). Tenant de prueba **demo** creado y aislado (`demo.yupipos.com/app`). Ver [[yupipos-vps-deploy]] para acceso, credenciales y **pendientes** (layout columnas por sección de los forms, probar guardados reales, hardening final, credenciales NubeFact/Izipay reales por tenant). **Bugs de producción que solo aparecen renderizando con APP_DEBUG=false (no en la auditoría estática): ya se cazaron y arreglaron varios (recursión en form de cotización, 500 en dashboard/costeo/pedidos, etc.) — al tocar forms Filament, verificar el render real en el tenant, no solo que compile.**

---

## Migraciones / despliegue

~61 archivos en `install/`. Al desplegar hay que aplicar las **nuevas** en phpMyAdmin (no hay tracking automático). **Para saber cuáles faltan: pega `install/check_migraciones.sql` en phpMyAdmin** → te lista cada migración con ✅/❌ (mirando si su columna/tabla existe). Las más recientes: `49_almacen_central.sql` (ubicaciones.es_almacen), `50_quotes_evento.sql`, `51_agenda_atendido.sql`, `52_quotes_venta.sql` (**sin uso** — quedó en agenda), `53_agenda_venta.sql`, `54_eventos_usa_pos.sql` (eventos.usa_pos DEFAULT 1), `55_gastos_v2.sql` (subcategorías + ítems de gasto), `56_mesas.sql` (mesa_pisos/mesas/mesa_elementos), `57_cuentas.sql` (cuentas + comandas de mesa + pedidos.cuenta_id/mesa_id + origen='mesa'), `58_cobro_mesas.sql` (cuenta_pagos + cuentas.precuenta_at/descuento/cobrada_at), `59_cuenta_mesas.sql` (agrupar mesas), `60_costeo_recetas.sql` (subrecetas + subreceta_items + receta_componentes [+backfill desde recetas] + receta_ficha), `61_subrecetas_stock.sql` (subrecetas.lleva_stock + subreceta_stock + subreceta_movimientos). Muchas tablas tienen guards (`inventarioListo()`, `cuentaPagosListo()`, `cuentaMesasListo()`, `$ready`, try/catch) para no romper si falta aplicar una.

---

## Pendiente / notas honestas

- **Estado al cierre (jun 2026) — DEPLOY PENDIENTE:** recién mergeado a `main` (sin aplicar en el server todavía): **Costeo Fase 1+2** (recetas con subrecetas + ficha + simulador food cost + dashboard de costeo), **subrecetas con stock/producción** y **boleta simple visible en el POS**. **Para retomar:** en el server `git pull` + aplicar en phpMyAdmin las migraciones **`60_costeo_recetas.sql`** y **`61_subrecetas_stock.sql`** (confirmar con `install/check_migraciones.sql` → #60 y #61 en ✅). Después **probar el circuito completo**: crear receta → subreceta con "lleva stock" → producir un lote (`admin/inventory/produccion.php`) → vender el producto → marcar **"Listo"** en el KDS (descuenta insumos + stock de subreceta) → ver el **dashboard de Costeo** y el **stock** (alerta de negativo). La boleta simple no necesita migración (solo `git pull`). Lo siguiente grande en cola es **PedidosYa** (abajo), bloqueado por credenciales.
- **Integración PedidosYa (delivery → POS)** — diseño aprobado y congelado en `docs/superpowers/specs/2026-06-23-pedidosya-integracion-design.md`; **construcción BLOQUEADA** hasta tener credenciales + doc de la API de PedidosYa (Delivery Hero; no hay API self-service — partnership/certificación o un agregador tipo Deliverect). Decidido: entra a la **bandeja** como WhatsApp (badge "PedidosYa", aceptar/rechazar hablan con su API, sin paso de cobro porque ya está pagado), pedido `origen='pedidosya'` (nuevo valor del enum), al "Listo" descuenta stock con las recetas, **boleta simple** por defecto / factura si trae RUC, comisión de PedidosYa → Gastos. Ya se generó y compartió la **llave PGP** que pedían (`daniel@elgringo.pe`).
...
- **SaaS "YupiPOS"** (yupipos.com) — producto aparte EN CONSTRUCCIÓN (Laravel, BD-por-tenant, white-label). Ver la sección **"Proyecto SaaS — YupiPOS"** arriba. No toca El Gringo.
