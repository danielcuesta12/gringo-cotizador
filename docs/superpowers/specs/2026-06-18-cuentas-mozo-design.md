...
- **Interruptor global (kill-switch):** setting `mozo_geocerca_activa` en `company_settings` (default `'1'`), editable por el admin en **Ajustes → tarjeta POS** (junto a `pos_nombre_obligatorio`). Si el admin lo pone en `'0'`, la geocerca del mozo se **apaga por completo** (la app vuelve a solo-PIN) — el escape para cuando el GPS falla y bloquea a todos. Es **independiente** de la geocerca de Asistencia (`ubicaciones.geocerca_activa`): apagar una no afecta la otra.
- **Configuración del punto:** el admin fija `lat/lng/geocerca_radio` por local (la pantalla de config de Asistencia ya lo hace; se reutilizan esas coordenadas). Si el local no tiene `lat/lng`, no se puede restringir → se permite y se recomienda configurarlo.
- **Helper:** `dentroGeocerca(int $ubicacionId, ?float $lat, ?float $lng): bool` en `includes/cuentas.php`. Devuelve `true` (permite) si `mozo_geocerca_activa='0'` **o** el local no tiene coordenadas; si está activo y el local tiene coordenadas, compara por haversine y devuelve `false` cuando está fuera del radio o faltan `$lat/$lng`. Lo invocan las escrituras de `api/mozo.php`.
