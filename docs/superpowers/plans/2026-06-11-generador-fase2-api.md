# Generador de cartas PDF · Fase 2 — API (`api/cartas.php`)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Crear `api/cartas.php` con todos los endpoints JSON que el editor necesita: CRUD de cartas/secciones/ítems, reordenar, cargar-desde-ubicación y subir-foto.

**Architecture:** Endpoint estilo Lima (`requireLogin()` + `Content-Type: application/json` + `$action` por switch). Las acciones de escritura exigen `verifyCsrf()` (acepta `X-CSRF-Token` o `csrf_token`). Lee/escribe en `cartas`/`carta_secciones`/`carta_items` con prepared statements; `cargar_ubicacion` usa transacción.

**Tech Stack:** PHP 8 (PDO via `Database`), JSON. Reusa `uploadImage()` para fotos.

**Rama:** `generador-cartas`. **Aislamiento:** archivo NUEVO; no toca `carta/banner.php`, `admin/locations/*`, ni `products` (solo los LEE en `cargar_ubicacion`). **Verificación:** `php -l` + review de seguridad/correctitud.

## Estructura de archivos

- Create: `api/cartas.php` — todos los endpoints del generador.

## Endpoints (acción vía `?action=`)

Lectura (GET): `list`, `get`.
Escritura (POST, con CSRF): `create`, `delete`, `save_meta`, `seccion_create`, `seccion_update`, `seccion_delete`, `seccion_reorder`, `item_create`, `item_update`, `item_delete`, `item_reorder`, `cargar_ubicacion`, `upload_foto`.

---

## Tarea 1: Crear `api/cartas.php`

**Files:** Create `api/cartas.php`.
...
Run: `php -l api/cartas.php`
Expected: `No syntax errors detected`.

- [ ] **Step 3: Commit**

```bash
git add api/cartas.php
git commit -m "feat(generador): api/cartas.php — CRUD cartas/secciones/items, cargar-ubicación, subir-foto"
```
...
- [ ] `php -l api/cartas.php` sin errores.
- [ ] Revisión de seguridad: todas las acciones de escritura llaman `verifyCsrf()`; `requireLogin()`+`isAdmin()` arriba; entradas con `clean/cleanInt/cleanFloat`; `foto` validada por whitelist regex; SQL siempre con prepared statements.
- [ ] `cargar_ubicacion` lee `location_products`/`products`/`categories` (no los modifica) y copia a `carta_secciones`/`carta_items` en transacción.
- [ ] `upload_foto` reusa `uploadImage($_FILES['foto'],'carta')` → guarda en `uploads/carta/`, devuelve ruta relativa.
- [ ] Aislamiento: el diff no toca `carta/banner.php`, `admin/locations/*`, ni `products`.

## Pendiente (su propio plan)

- **Fase 3:** `admin/cartas/index.php` (lista/nueva/borrar) + `admin/cartas/editor.php` (editor de 2 paneles que consume esta API) + entrada en el sidebar.
